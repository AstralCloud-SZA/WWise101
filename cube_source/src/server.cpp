    ...
    if(enet_host_service(serverhost, &event, timeout) > 0)
    {
        switch(event.type)
        {
            ...
            case ENET_EVENT_TYPE_RECEIVE:
                ...
                process(event.packet, (AkIntPtr)event.peer->data); 
                if(event.packet->referenceCount==0) enet_packet_destroy(event.packet);
                break;

            case ENET_EVENT_TYPE_DISCONNECT: 
                if((AkIntPtr)event.peer->data<0) break;
                printf("disconnected client (%s)\n", clients[(AkIntPtr)event.peer->data].hostname);
                clients[(AkIntPtr)event.peer->data].type = ST_EMPTY;
                send2(true, -1, SV_CDIS, (AkIntPtr)event.peer->data);

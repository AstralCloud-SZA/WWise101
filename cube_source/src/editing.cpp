...
void setwwiseenv(int envID)
{
    EDITSELMP;

	for(int x = sel.x; x<sel.xs+sel.x; x++) for(int y = sel.y; y<sel.ys+sel.y; y++) S(x,y)->env = envID;
}

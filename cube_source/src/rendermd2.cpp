...
struct md2
{
    ...
    void render(vec &light, int numFrame, int range, float x, float y, float z, float yaw, float pitch, float scale, float speed, int snap, int basetime, dynent * ent = NULL, bool hellpig = false );
...
void md2::render(vec &light, int frame, int range, float x, float y, float z, float yaw, float pitch, float sc, float speed, int snap, int basetime, dynent * ent, bool hellpig )
{
    ...
    else
    {
		...
		float time = lastmillis-basetime;
		...
		// super footstep sound hack job.

		if ( ent )
		{
			if ( ent->lastanimframe != fr2 )
			{
				if ( hellpig )
				{
					if ( frame == 1 )
				...
				else
				{
					if ( frame == 40 ) // running
					{
						if ( fr2 == 41 // left foot
							|| fr2 == 44 ) // right foot )
						{
								monsterfootstep( ent, fr2 == 44 );
						}
					}
				}
				ent->lastanimframe = fr2;
			}
		}
...
void rendermodel(char *mdl, int frame, int range, int tex, float rad, float x, float y, float z, float yaw, float pitch, bool teammate, float scale, float speed, int snap, float basetime, dynent * ent, bool hellpig  )
{
    md2 *m = loadmodel(mdl);
    ...
    m->render(light, frame, range, x, y, z, yaw, pitch, scale, speed, snap, (int)basetime, ent, hellpig );
};

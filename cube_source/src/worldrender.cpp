...
{
	float _d1 = d1->vdelta*0.25f;
	float _d2 = d2->vdelta*0.25f;

    if(SOLID(o) || o->type==SEMISOLID)
    {
        float c1 = s->floor;
        float c2 = s->floor;
        if(s->type==FHF) { c1 -= _d1; c2 -= _d2; };
        float f1 = s->ceil;
        float f2 = s->ceil;
        if(s->type==CHF) { f1 += _d1; f2 += _d2; };
...
void render_seg_new(float vx, float vy, float vh, int mip, int x, int y, int xs, int ys)
{
    ...
    #define df(x) s->floor-(x->vdelta*0.25f)
    #define dc(x) s->ceil+(x->vdelta*0.25f)
    ...
    LOOPH continue;     // floors
        ...
        if(s->type==FHF)
        {
			...
			if(df(s) < hdr.waterlevel && !SOLID(s)) addwaterquad(xx<<mip, yy<<mip, 1<<mip);
        };
    }};
